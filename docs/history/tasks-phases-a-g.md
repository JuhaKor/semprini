# TASKS.md history — Phases A to G

The completed phases of `TASKS.md`, moved here verbatim on 2026-09-16 so that the live
file holds only open work. Every task below is done; its notes record what the session
that did it found and decided, and are the only record of some of those decisions (A2's
w3id routing, for one). Section references (`§`) are to
`docs/rdf-repo-and-compiler-spec.md`; task references (`B1`, `F3`) are to this file.

The sequencing notes at the end are the ones written while these phases were open, kept
for the same reason. `TASKS.md` carries the notes for what is still open.
